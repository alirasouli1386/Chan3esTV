...
@section( 'content' )
	<div class="container">
		...
		<div class="row">
			<div class="col-md-9">
				<h4>{{ $modify == 1 ? 'Modify Kids' : 'New Kids' }}</h4>
			</div>
		</div>
		<hr/>
		<br/>
		...
		<section class="row">
			<form class="w-100" action="{{ $modify == 1 ? route('update_kids', [ 'kids_id' => $kids_id ]) : route('create_kids') }}" method="post" enctype="multipart/form-data">
				{{csrf_field()}}
					...
					<div class="row">
						<div class="col-md">
							<div class="input-group">
								...
								<input class="form-control" name="age_limit" type="number" min="0" value="{{ old('age_limit') ? old('age_limit') : $age_limit }}" required/>
								<small class="error"><br/>{{$errors->first('age_limit')}}</small>
							</div>
						</div>
						<div class="col-md">
							<div class="input-group">
				...
				<div class="form-group col-md-4 offset-md-2">
					<button class="btn {{$modify==1?'btn-warning':'btn-success'}}" type="submit">{{$modify==1?'Modify Kids':'Save Kids'}}</button>
					<a class="btn btn-info" href="{{route('manage_kids')}}" type="button">Close</a>
				</div>
			</form>
...
@section( 'body_scripts' )
	...
	$( document ).ready( function () {
		if ( fileExists( location.protocol + '//' + location.hostname + ( location.port ? ':' + location.port : '' ) + '/img/tv/kids/{{$thumbnail_image_route}}' ) ) {
			$( '#thumbnail_image' ).text( 'Thumbnail exists; Replace:' );
		}

		if ( fileExists( location.protocol + '//' + location.hostname + ( location.port ? ':' + location.port : '' ) + '/img/tv/kids/{{$cover_image_route}}' ) ) {
			$( '#cover_image' ).text( 'Cover exists; Replace:' );
		}

		if ( fileExists( location.protocol + '//' + location.hostname + ( location.port ? ':' + location.port : '' ) + '/img/tv/kids/{{$background_image_route}}' ) ) {
