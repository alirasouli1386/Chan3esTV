...
@section( 'content' )
	<div class="container">
		...
		<div class="row">
			<div class="col-md-9">
				<h4>{{ $modify == 1 ? 'Modify Documentary' : 'New Documentary' }}</h4>
			</div>
		</div>
		<hr/>
		<br/>
		...
		<section class="row">
			<form class="w-100" action="{{ $modify == 1 ? route('update_documentary', [ 'documentary_id' => $documentary_id ]) : route('create_documentary') }}" method="post" enctype="multipart/form-data">
				{{csrf_field()}}
				...
				<div class="form-group col-md-4 offset-md-2">
					<button class="btn {{$modify==1?'btn-warning':'btn-success'}}" type="submit">{{$modify==1?'Modify Documentary':'Save Documentary'}}</button>
					<a class="btn btn-info" href="{{route('manage_documentaries')}}" type="button">Close</a>
				</div>
			</form>
...
@section( 'body_scripts' )
	<script>
		...
		$( document ).ready( function () {
			if ( fileExists( location.protocol + '//' + location.hostname + ( location.port ? ':' + location.port : '' ) + '/img/tv/documentaries/{{$thumbnail_image_route}}' ) ) {
				$( '#thumbnail_image' ).text( 'Thumbnail exists; Replace:' );
			}

			if ( fileExists( location.protocol + '//' + location.hostname + ( location.port ? ':' + location.port : '' ) + '/img/tv/documentaries/{{$cover_image_route}}' ) ) {
