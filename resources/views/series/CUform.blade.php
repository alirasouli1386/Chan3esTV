...
@section( 'content' )
	<div class="container">
		...
		<div class="row">
			<div class="col-md-9">
				<h4>{{ $modify == 1 ? 'Modify Series' : 'New Series' }}</h4>
			</div>
		</div>
		<hr/>
		<br/>
		...
		<section class="row">
			<form class="w-100" action="{{ $modify == 1 ? route('update_series', [ 'series_id' => $series_id ]) : route('create_series') }}" method="post" enctype="multipart/form-data">
				{{csrf_field()}}
					...
					<div class="row">
						<div class="col-md">
							<div class="input-group">
								...
								<input class="form-control" name="seasons" type="number" min="0" value="{{ old('seasons') ? old('seasons') : $seasons }}" required/>
								<small class="error"><br/>{{$errors->first('seasons')}}</small>
							</div>
						</div>
						<div class="col-md">
							<div class="input-group">
					...
					<div class="row">
						...
						<div class="col-md">
							<div class="input-group" style="direction: rtl">
								...
								<input class="form-control farsi" name="actorsfa" type="text" value="{{ old('actors_fa') ? old('actors_fa') : $actors_fa }}"/>
				...
				<div class="form-group">
					<div class="input-group">
						...
						<textarea class="form-control" name="description_au">
							{{ old('description_au') ? old('description_au') : $description_au }}
						</textarea>
				...
				<div class="form-group">
					<div class="input-group" style="direction: rtl">
						...
						<textarea class="form-control farsi" name="mini_story_fa">
							{{ old('mini_story_fa') ? old('mini_story_fa') : $mini_story_fa }}
						</textarea>
				...
				<div class="form-group">
					<div class="input-group" style="direction: rtl">
						...
						<textarea class="form-control farsi" name="description_fa">
							{{ old('description_fa') ? old('description_fa') : $description_fa }}
						</textarea>
				...
				<div class="form-group col-md-4 offset-md-2">
					<button class="btn {{$modify==1?'btn-warning':'btn-success'}}" type="submit">{{$modify==1?'Modify Series':'Save Series'}}</button>
					<a class="btn btn-info" href="{{route('manage_series')}}" type="button">Close</a>
				</div>
			</form>
...
@section( 'body_scripts' )
	...
	$( document ).ready( function () {
		if ( fileExists( location.protocol + '//' + location.hostname + ( location.port ? ':' + location.port : '' ) + '/img/tv/series/{{$thumbnail_image_route}}' ) ) {
			$( '#thumbnail_image' ).text( 'Thumbnail exists; Replace:' );
		}

		if ( fileExists( location.protocol + '//' + location.hostname + ( location.port ? ':' + location.port : '' ) + '/img/tv/series/{{$cover_image_route}}' ) ) {
			$( '#cover_image' ).text( 'Cover exists; Replace:' );
		}

		if ( fileExists( location.protocol + '//' + location.hostname + ( location.port ? ':' + location.port : '' ) + '/img/tv/series/{{$background_image_route}}' ) ) {
