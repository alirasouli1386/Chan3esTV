...
class MovieController extends Controller
{
  ...
  public function index() {
    $data = [];
    $data[ 'movies' ] = $this->movie->all()->where( 'is_archived', 0 );
    return view( 'movie/index', $data );
  }

  public function show( $movie_id ) {
    $content_data = $this->movie->find( $movie_id );
    ...
    if ($content_data !=null) {
      ...
      $data[ 'thumbnail_image_route' ] = 'img/tv/movies/' . $content_data->thumbnail_image_route;

      if ($content_data->trailer_route!="" && file_exists( public_path() . '/tv/movies/' . $content_data->trailer_route ) ) {
        $data[ 'cover_route' ] = 'tv/movies/' . $content_data->trailer_route;
        $data[ 'cover_type' ] = 'video';
      } else if($content_data->cover_image_route!="" && file_exists( public_path() . '/img/tv/movies/' . $content_data->cover_image_route )) {
        $data[ 'cover_route' ] = 'img/tv/movies/' . $content_data->cover_image_route;
      ...
      if($content_data->background_image_route!="" && file_exists( public_path() . '/img/tv/movies/' . $content_data->background_image_route )) {
        $data[ 'background_image_route' ] = 'img/tv/movies/' . $content_data->background_image_route;
      } else if (file_exists( public_path() . '/img/tv/defaults/movie_background.png' )) {
        $data[ 'background_image_route' ] = 'img/tv/defaults/movie_background.png';
      } else {
  ...
  public function manage() {
    $data = [];
    $data[ 'contents' ] = $this->movie->all();
    return view( 'movie/manage', $data );
  }
  ...
  public function create( Request $request, Movie $movie ) {
    ...
    if ( $request->isMethod( 'post' ) ) {
      $this->validate(
        ...
        $request->thumbnail_image_route->move( public_path() . '/img/tv/movies/', $data[ 'thumbnail_image_route' ] );

        $data[ 'cover_image_route' ] = ( ( string )Str::uuid() ) . '.' . ( $request->cover_image_route->getClientOriginalExtension() );
        $request->cover_image_route->move( public_path() . '/img/tv/movies/', $data[ 'cover_image_route' ] );
        ...
        if($data[ 'background_image_route' ]!=null){
          ...
          $request->background_image_route->move( public_path() . '/img/tv/movies/', $data[ 'background_image_route' ] );
        }
        else {
        ...
        return redirect( '/managetv/movies' );
      }

      $data[ 'modify' ] = 0;

      return view( 'movie/CUform', $data );
    }
    ...
    public function modify( $movie_id, Request $request, Movie $movie ) {
      $data = [];
      $data[ 'name_exclusive' ] = $request->name_exclusive;
      $data[ 'groupname_au' ] = $request->groupname_au?$request->groupname_au:'';
      $data[ 'groupname_fa' ] = $request->groupname_fa?$request->groupname_fa:'';
      ...
      if ( $request->isMethod( 'post' ) ) {
        ...
        if ( $request->thumbnail_image_route != null ) {
          if ( file_exists( public_path() . '/img/tv/movies/' . $content_data->thumbnail_image_route ) ) {
            unlink( public_path() . '/img/tv/movies/' . $content_data->thumbnail_image_route );
          }
          $content_data->thumbnail_image_route = ( ( string )Str::uuid() ) . '.' . ( $request->thumbnail_image_route->getClientOriginalExtension() );
          $request->thumbnail_image_route->move( public_path() . '/img/tv/movies/', $content_data->thumbnail_image_route );
        }

        if ( $request->cover_image_route != null ) {
          if ( file_exists( public_path() . '/img/tv/movies/' . $content_data->cover_image_route ) ) {
            unlink( public_path() . '/img/tv/movies/' . $content_data->cover_image_route );
          }
          $content_data->cover_image_route = ( ( string )Str::uuid() ) . '.' . ( $request->cover_image_route->getClientOriginalExtension() );
          $request->cover_image_route->move( public_path() . '/img/tv/movies', $content_data->cover_image_route );
        }

        if ( $request->background_image_route != null ) {
          if ( file_exists( public_path() . '/img/tv/movies/' . $content_data->background_image_route ) ) {
            unlink( public_path() . '/img/tv/movies/' . $content_data->background_image_route );
          }
          $content_data->background_image_route = ( ( string )Str::uuid() ) . '.' . ( $request->background_image_route->getClientOriginalExtension() );
          $request->background_image_route->move( public_path() . '/img/tv/movies', $content_data->background_image_route );
        }
    ...
    public function toggle_archive( $movie_id, Movie $movie ) {
      $content_data = $this->movie->find( $movie_id );
    ...
    public function destroy( $movie_id, Movie $movie ) {
      $content_data = $this->movie->find( $movie_id );

      if ($content_data != null) {
        if ($content_data->thumbnail_image_route!="" && file_exists( public_path() . '/img/tv/movies/' . $content_data->thumbnail_image_route ) ) {
          unlink( public_path() . '/img/tv/movies/' . $content_data->thumbnail_image_route );
        }
        if ($content_data->cover_image_route!="" && file_exists( public_path() . '/img/tv/movies/' . $content_data->cover_image_route ) ) {
          unlink( public_path() . '/img/tv/movies/' . $content_data->cover_image_route );
        }
        if ($content_data->background_image_route!="" && file_exists( public_path() . '/img/tv/movies/' . $content_data->background_image_route ) ) {
          unlink( public_path() . '/img/tv/movies/' . $content_data->background_image_route );
        ...
        if ($content_data->trailer_route!="" && file_exists( public_path() . 'tv/movies/' . $content_data->trailer_route ) ) {
          unlink( public_path() . 'tv/movies/' . $content_data->trailer_route );
        }

        $this->movie->destroy( $movie_id );
      }
